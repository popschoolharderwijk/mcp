...
describe('RLS: teacher_lesson_types SELECT', () => {
	...
	it('site_admin sees all lesson type links', async () => {
		...
		const data = unwrap(await db.from('teacher_lesson_types').select('*'));
	...
	it('teacher cannot see other teachers lesson type links', async () => {
		...
		const data = unwrap(await db.from('teacher_lesson_types').select('*').eq('teacher_user_id', bobTeacherUserId));
